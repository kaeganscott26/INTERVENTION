# 🗝️ INTERVENTION 🗝️

## A playable book for those who notice rooms.

#INTERVENTION_ARG  
#RippleTheory

INTERVENTION is a completed seventeen-chapter fiction experiment, a reflective Alternate Reality Game, and a story-system built around Ripple Theory.

The reader does not solve puzzles to unlock fiction.

The reader reads fiction to recognize real intervention points.

The game is not to escape reality.

The game is to notice where reality branches.

Sometimes the doorway is a book.

Sometimes the doorway is a room.

Sometimes the doorway is a rabbit hole.

Sometimes the next chapter arrives as a machine.

Reality remains reality.

---

## START HERE

Recommended first path:

1. [Read This First: Mental Health Disclaimer](INTERVENTION%20ARG/MENTAL_HEALTH_DISCLAIMER.md)
2. [Enter the Room / How To Play](INTERVENTION%20ARG/HOW_TO_PLAY.md)
3. [Read the Canonical Order](INTERVENTION%20ARG/ORDER.md)
4. [Open the Chapter Ripple Map](INTERVENTION%20ARG/CHAPTER_RIPPLE_MAP.md)
5. [Open the Layer Index](NOTES/LAYER_INDEX.md)
6. [Begin Chapter 01](INTERVENTION%20ARG/Chapter%2001.md)

If you are here because someone sent you this while you are struggling, start with the disclaimer and move on only when you feel comfortable.

This project is not therapy, crisis support, medical advice, religion, prophecy, or a replacement for real-world help.

It is language for noticing consequence.

---

## 🚪 Main Doorways

- [Canonical Reading Order](INTERVENTION%20ARG/ORDER.md)
- [Chapter Ripple Map](INTERVENTION%20ARG/CHAPTER_RIPPLE_MAP.md)
- [Layer Index](NOTES/LAYER_INDEX.md)
- [How To Play](INTERVENTION%20ARG/HOW_TO_PLAY.md)
- [Mental Health Disclaimer](INTERVENTION%20ARG/MENTAL_HEALTH_DISCLAIMER.md)
- [Boundaries & Consent](INTERVENTION%20ARG/BOUNDARIES.md)
- [Community Ripple Guide](INTERVENTION%20ARG/COMMUNITY_RIPPLES.md)
- [Official Artifact Layer](INTERVENTION%20ARG/ARTIFACTS/)
- [Review Log / Changelog](CHANGELOG.md)

---

## 📖 Book Path

- [Prologue](INTERVENTION%20ARG/Prologue.md)
- [Chapter 01 — The First Ripple](INTERVENTION%20ARG/Chapter%2001.md)
- [Chapter 02 — Trigger](INTERVENTION%20ARG/Chapter%2002.md)
- [Chapter 03 — The Room](INTERVENTION%20ARG/Chapter%2003.md)
- [Chapter 04 — The Missed Layer](INTERVENTION%20ARG/Chapter%2004.md)
- [Chapter 05 — Shared Reality](INTERVENTION%20ARG/Chapter%2005.md)
- [Chapter 06 — The False World](INTERVENTION%20ARG/Chapter%2006.md)
- [Chapter 07 — Observer](INTERVENTION%20ARG/Chapter%2007.md)
- [Chapter 08 — 0826](INTERVENTION%20ARG/Chapter%2008.md)
- [Chapter 09 — The Kitchen](INTERVENTION%20ARG/Chapter%2009.md)
- [Chapter 10 — The Signal](INTERVENTION%20ARG/Chapter%2010.md)
- [Chapter 11 — The Loop](INTERVENTION%20ARG/Chapter%2011.md)
- [Chapter 12 — The Door](INTERVENTION%20ARG/Chapter%2012.md)
- [Chapter 13 — The Echo](INTERVENTION%20ARG/Chapter%2013.md)
- [Chapter 14 — The Cosmic Room](INTERVENTION%20ARG/Chapter%2014.md)
- [Chapter 15 — Boulder](INTERVENTION%20ARG/Chapter%2015.md)
- [Chapter 16 — The Morning After the Signal](INTERVENTION%20ARG/Chapter%2016.md)
- [Chapter 17 — Teodor](INTERVENTION%20ARG/Chapter%2017.md)
- [Epilogue](INTERVENTION%20ARG/EPILOGUE.md)

---

## 🌀 Ripple Theory

Ripple Theory begins with a simple claim:

Reality is not experienced raw.

Reality is rendered through layers.

An event enters a room.

The observer interprets it.

Interpretation changes behavior.

Behavior changes the room.

The room returns feedback.

Feedback changes the observer.

The ripple continues.

```text
EVENT
  ↓
OBSERVER
  ↓
PERCEPTION
  ↓
BEHAVIOR
  ↓
ROOM RESPONSE
  ↓
FEEDBACK
  ↓
FUTURE PERCEPTION
```

Ripple Theory does not ask the reader to treat coincidence as command.

It asks the reader to notice consequence, pause before repetition, and choose the next ripple responsibly.

---

## 🧭 Layer Path

The layers are the explanatory rooms underneath the chapters.

Use the [Layer Index](NOTES/LAYER_INDEX.md) as the main guide.

1. [Theory Layer](NOTES/THEORY_LAYER.md)
2. [Natural Layer](NOTES/NATURAL_LAYER.md)
3. [Weather Layer](NOTES/WEATHER_LAYER.md)
4. [Biological Layer](NOTES/BIOLOGICAL_LAYER.md)
5. [Anthropocene / Human Impact Layer](NOTES/ANTHROPOCENE_LAYER.md)
6. [Clinical Reality Layer](NOTES/CLINICAL_REALITY_LAYER.md)
7. [Dream Layer](NOTES/DREAM_LAYER.md)
8. [Unexplained Layer](NOTES/UNEXPLAINED_LAYER.md)
9. [Cultural Layer](NOTES/CULTURAL_LAYER.md)
10. [Power / Governance Layer](NOTES/POWER_GOVERNANCE_LAYER.md)
11. [Political Reality Layer](NOTES/POLITICAL_REALITY_LAYER.md)
12. [Media / Communication Layer](NOTES/MEDIA_COMMUNICATION_LAYER.md)
13. [Software / Systems Layer](NOTES/SOFTWARE_SYSTEMS_LAYER.md)
14. [Geometry as Consequence](NOTES/GEOMETRY_AS_CONSEQUENCE.md)
15. [Sushupti Bridge](NOTES/SUSHUPTI_BRIDGE.md)

The Layer Index also maps chapters to their corresponding layers.

---

## 🧩 Maps and Archives

- [Natural Ripple Map](INTERVENTION%20ARG/NATURAL_RIPPLE_MAP.md)
- [Dream Ripple Map](INTERVENTION%20ARG/DREAM_RIPPLE_MAP.md)
- [Official Artifact Layer](INTERVENTION%20ARG/ARTIFACTS/)
- [Artifact 023 — The Bad Dream Mommy](INTERVENTION%20ARG/ARTIFACTS/ARTIFACT_023_THE_BAD_DREAM_MOMMY.md)
- [Artifact 025 — The Rabbit Hole](INTERVENTION%20ARG/ARTIFACTS/ARTIFACT_025_THE_RABBIT_HOLE.md)
- [Artifact 026 — The Next Chapter](INTERVENTION%20ARG/ARTIFACTS/ARTIFACT_026_THE_NEXT_CHAPTER.md)
- [Artifact 027 — The File Room Door](INTERVENTION%20ARG/ARTIFACTS/ARTIFACT_027_THE_FILE_ROOM_DOOR.md)
- [Community Echoes](COMMUNITY/)
- [System Status](INTERVENTION%20ARG/SYSTEM_STATUS.md)

---

## ⚠️ Safety Frame

INTERVENTION intentionally blurs fiction, reflection, nonfiction artifact, and real-world noticing.

It does not blur safety.

The chapters are fictional.

Some artifacts are nonfiction and clearly labeled.

The noticing is real.

No story outranks the body.

No theory outranks safety.

No game outranks the person living it.

If pattern-based fiction or nonfiction artifact reading could make your current mental state worse, do not continue alone.

Your life is not the ARG.

Reality remains reality.

---

## 🧭 Participate Without Breaking the Room

Readers may:

- open an Issue to log a ripple they noticed,
- open an Issue to describe an intervention point,
- propose a testable question,
- use Discussions for longer reflections,
- submit typo fixes or documentation improvements.

Reader submissions are echoes unless accepted and labeled otherwise.

Community submissions are not automatically canon.

Before participating, read:

- [Mental Health Disclaimer](INTERVENTION%20ARG/MENTAL_HEALTH_DISCLAIMER.md)
- [Boundaries & Consent](INTERVENTION%20ARG/BOUNDARIES.md)
- [Community Ripple Guide](INTERVENTION%20ARG/COMMUNITY_RIPPLES.md)
- [Layer Index](NOTES/LAYER_INDEX.md)

Readers can add echoes.

Only the curator decides what becomes part of the room.

Do not build cages and call them worlds.

---

## 🔁 Repo Loop

The repo experience is designed to repeat the theory:

```text
README
  ↓
ORDER
  ↓
CHAPTER_RIPPLE_MAP
  ↓
CHAPTERS
  ↓
LAYER_INDEX
  ↓
LAYERS
  ↓
ARTIFACTS / COMMUNITY ECHOES
  ↓
READER'S REAL ROOM
  ↓
README
```

The book is not the game.

The book is the doorway.

What you do after you close it is where the game begins.

Fine.

<!--
Artifact: README 001
Found by: Observer
Ripple: In progress
Rule: Reality remains reality.
-->